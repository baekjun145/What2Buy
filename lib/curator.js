...
async function curate(scored, context) {
  if (!isConfigured()) return { items: scored, source: "score", detail: "no_key" };
  if (scored.length === 0) return { items: scored, source: "score", detail: "no_candidates" };
  ...
  try {
    const response = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        model: MODEL,
        // 같은 조건으로 다시 물었을 때 결과가 들쭉날쭉하면 신뢰가 떨어진다.
        // 낮게 잡아 되도록 같은 답이 나오게 한다.
        temperature: 0.1,
    ...
    if (!response.ok) {
      ...
      return { items: scored, source: "score", detail: `http_${response.status}:${body.slice(0, 80)}` };
    }

    const data = await response.json();
    const picks = parsePicks(data?.choices?.[0]?.message?.content);
    if (picks.length === 0) return { items: scored, source: "score", detail: "no_picks" };
    ...
    if (chosen.length === 0) return { items: scored, source: "score", detail: "name_mismatch" };
    ...
    return { items: scored, source: "score", detail: `err_${error.name}:${String(error.message).slice(0, 60)}` };
  } finally {
    clearTimeout(timer);
  }
}
